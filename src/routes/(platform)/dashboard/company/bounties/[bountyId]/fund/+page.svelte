...
<div class="mx-auto max-w-2xl space-y-6">
	<header class="space-y-1">
		<a href="/dashboard/company/bounties" class="text-ink-soft text-sm hover:underline">
			&larr; Back to bounties
		</a>
		<h1 class="fow-display text-ink text-3xl">Fund bounty</h1>
		<p class="text-ink-soft text-sm">{data.bounty.title}</p>
	</header>
	...
	<Card>
		...
		<CardContent class="space-y-1.5">
			{#if data.bounty.prizeTiers.length === 0}
				<p class="text-ink-soft text-sm">No prize tiers configured.</p>
			{:else}
					...
					{#each data.bounty.prizeTiers as tier (tier.id)}
						<li class="flex items-center justify-between py-2">
							<span class="flex items-center gap-2">
								...
								{/if}
								<span class="text-ink">{tier.label ?? `Position ${tier.position}`}</span>
							</span>
							<span class="font-medium tabular-nums"
								>{formatMoney(tier.amount, data.bounty.currency)}</span
							>
						</li>
					{/each}
				</ul>
		...
		<CardFooter class="border-bone flex items-center justify-between border-t pt-4">
			<span class="text-ink-soft text-sm font-medium">Total to deposit</span>
			<span class="text-lg font-semibold tabular-nums"
				>{formatMoney(totalMinor, data.bounty.currency)}</span
			>
		</CardFooter>
	</Card>
