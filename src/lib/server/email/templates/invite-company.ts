...
export function inviteCompany({ email, companyName, url }: InviteCompanyProps) {
	const subject = "You've been invited to Learn2Earn";
	const greeting = companyName
		? `Hi from Learn2Earn — your company "${companyName}" has been invited.`
		: 'Hi from Learn2Earn — you have been invited.';
	return {
		subject,
		text: `${greeting}\n\nSet your password and complete onboarding here:\n${url}\n\nIf you weren't expecting this email, you can ignore it.`,
		html: `<p>${escapeHtml(greeting)}</p>
<p>You've been invited to join Learn2Earn as a company at <strong>${escapeHtml(email)}</strong>.</p>
