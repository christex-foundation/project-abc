<script lang="ts">
	...
	// Read-only: the handle is auto-generated; we only surface the link + share
	// actions. `name` seeds the share message text.
	...
	async function copyLink() {
		try {
			...
			// Clipboard blocked (insecure context / permissions) — link stays visible.
		}
	}
	...
	// Brand icons (Lucide dropped social logos) — single-path SVGs from simple-icons.
...
<Card>
	...
	<CardContent class="space-y-4">
		...
		<div class="flex flex-wrap items-center gap-2">
			<span class="text-ink-soft text-xs font-medium">Share to</span>
			{#if canNativeShare}
				<button
					type="button"
					onclick={nativeShare}
					aria-label="Share"
					title="Share"
					class="border-bone text-ink-soft hover:border-ink/40 hover:text-ink flex h-10 w-10 items-center justify-center rounded-full border transition-colors"
				>
			...
			{#each targets as t (t.name)}
				<a
					...
					class="border-bone hover:border-ink/30 flex h-10 w-10 items-center justify-center rounded-full border transition-colors"
					style={`color:${t.color}`}
				>
